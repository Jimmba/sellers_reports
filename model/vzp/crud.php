 ...
 */
    ...
    if (isset($_POST['modal']) && ($_POST['modal'])!= null){
        ...
        $modal = new Modal();
        echo $modal->init($modalAction, "vzp");
    }else{
        ...
        if ($action == "add") {
            ...
            $dsm = $dsm->getId();
            $id=null;
            $query = "Insert INTO v_zp values (?,?,?,?,?)";
            $data = array($id, $dsm, $sum, $prichina, $sotr);
        ...
        if ($action == "delete") {
            ...
            $id = $_POST['id'];
            $db->setQuery("DELETE FROM v_zp WHERE id_v_zp = $id");
            $result = $db->doQuery();
        ...
        if ($action == "change") {
            ...
            $query = "UPDATE v_zp SET dsm_id_dsm=?, `vzp(x100)`=?, sotrudniki_id_prod= ?, osnovanie=? WHERE id_v_zp=?";
