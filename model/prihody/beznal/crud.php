 ...
 */
    ...
    if (isset($_POST['modal']) && ($_POST['modal'])!= null){
        ...
        $modal = new Modal();
        echo $modal->init($modalAction, "prihody/beznal");
    }else{
        ...
        if ($action == "add") {
            ...
            $dsm = $dsm->getId();
            $id=null;
            $query = "Insert INTO prihBeznal values (?,?,?)";
        ...
        if ($action == "delete") {
            ...
            $id = $_POST['id'];
            $db->setQuery("DELETE FROM prihBeznal WHERE id_prih = $id");
            $result = $db->doQuery();
        ...
        if ($action == "change") {
            ...
            $query = "UPDATE prihBeznal SET dsm_id_dsm=?, `beznal(x100)`=? WHERE id_prih=?";
