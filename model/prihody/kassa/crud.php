 ...
 */
    ...
    if (isset($_POST['modal']) && ($_POST['modal'])!= null){
        ...
        $modal = new Modal();
        echo $modal->init($modalAction, "prihody/kassa");
    }else{
        ...
        if ($action == "add") {
            ...
            $dsm = $dsm->getId();
            $id=null;
            $query = "Insert INTO prihProbito values (?,?,?)";
        ...
        if ($action == "delete") {
            ...
            $id = $_POST['id'];
            $db->setQuery("DELETE FROM prihProbito WHERE id_prih = $id");
            $result = $db->doQuery();
        ...
        if ($action == "change") {
            ...
            $query = "UPDATE prihProbito SET dsm_id_dsm=?, `probito+cart(x100)`=? WHERE id_prih=?";
