 ...
 */
    ...
    if (isset($_POST['modal']) && ($_POST['modal'])!= null){
        ...
        $modal = new Modal();
        echo $modal->init($modalAction, "prihody/nal");
    }else{
        ...
        if ($action == "add") {
            ...
            $dsm = $dsm->getId();
            $id=null;
            $query = "Insert INTO prihNal values (?,?,?)";
        ...
        if ($action == "delete") {
            ...
            $id = $_POST['id'];
            $db->setQuery("DELETE FROM prihNal WHERE id_prih = $id");
            $result = $db->doQuery();
        ...
        if ($action == "change") {
            ...
            $query = "UPDATE prihNal SET dsm_id_dsm=?, `nal(x100)`=? WHERE id_prih=?";
