 ...
 */
    ...
    if (isset($_POST['modal']) && ($_POST['modal'])!= null){
        ...
        $modal = new Modal();
        echo $modal->init($modalAction, "rashody");
    }else{
        ...
        if ($action == "add") {
            ...
            $dsm = $dsm->getId();
            $id=null;
            $query = "Insert INTO rashod values (?,?,?,?)";
            $data = array($id, $dsm, $sum, $nachto);
        ...
        if ($action == "change") {
            ...
            $query = "UPDATE rashod SET dsm_id_dsm=?, `rashod(x100)`=?, kuda= ? WHERE id_rashod=?";
            $data = array($dsm, $sum, $nachto, $id);
            $STH=$DBH->prepare($query);
            $STH->execute($data);
        }
