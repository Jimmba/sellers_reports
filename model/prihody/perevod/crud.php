 ...
 */
    ...
    if (isset($_POST['modal']) && ($_POST['modal'])!= null){
        ...
        $modal = new Modal();
        echo $modal->init($modalAction, "prihody/perevod");
    }else{
        ...
        if ($action == "add") {
            ...
            $dsm = $dsm->getId();
            $id=null;
            $query = "Insert INTO prihPerevod values (?,?,?)";
        ...
        if ($action == "delete") {
            ...
            $id = $_POST['id'];
            $db->setQuery("DELETE FROM prihPerevod WHERE id_prih = $id");
            $result = $db->doQuery();
        ...
        if ($action == "change") {
            ...
            $query = "UPDATE prihPerevod SET dsm_id_dsm=?, `perevod_sto(x100)`=? WHERE id_prih=?";
